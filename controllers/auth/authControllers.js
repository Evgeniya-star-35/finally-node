...
class AuthControllers {
  ...
  //   const isUserExist = await authService.isExistUser(userData.data.email);

  //   if (!isUserExist) {
  //     await authService.createUser({ email: userData.data.email });
  //   }

  //   const currentUser = await User.findOne({ email: userData.data.email });

  //   return res.redirect(
  //     `${process.env.FRONTEND_URL}?token=${currentUser.token}`
  //   );
